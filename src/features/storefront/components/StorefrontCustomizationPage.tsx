...
const MODULE_TEMPLATES: ModuleTemplate[] = [
  ...
  },
  {
    ...
    description: 'Display your business location and contact details',
    icon: 'LocationOn',
    category: 'information',
    defaultSettings: {
    ...
    requiredSettings: ['showLocationName', 'showFullAddress'],
    previewComponent: 'BusinessAddressPreview',
    configComponent: 'BusinessAddressConfig',
  },
];
...
const StorefrontCustomizationPage: React.FC = () => {
  ...
  // Featured products manager dialog state
  const [featuredProductsManagerOpen, setFeaturedProductsManagerOpen] =
    useState(false);
    ...
    const loadStorefrontCustomization = async () => {
      ...
      try {
        ...
        if (customization) {
          ...
          if (customization.themeId || customization.theme?.id) {
            const themeId = customization.themeId || customization.theme?.id;
  ...
  const populateModuleSettings = useCallback(
    (storeData: Store) => {
      ...
      if (!storeData) {
        console.warn(
          '⚠️ storeData is null or undefined, skipping module population'
        );
        return;
      }
      ...
      setModules((prevModules) =>
        prevModules.map((module) => {
          ...
          if (module.type === 'contact-form') {
            ...
            const businessAddress = Array.isArray(storeData?.addresses)
          ...
          if (module.type === 'policy-section') {
            ...
            const allDaysHours = Array.from({ length: 7 }, (_, dayIndex) => {
              const existingHour = Array.isArray(storeData?.openHours)
                ? storeData.openHours.find(
            ...
            const hasFarmPickup = Array.isArray(storeData?.addresses)
            ...
            const logisticsInfo = {
              ...
              hasBusinessAddress: Array.isArray(storeData?.addresses)
                ? storeData.addresses.some(
                    (addr: any) =>
            ...
            updatedModule.settings = {
              ...
              paymentMethods: Array.isArray(storeData?.paymentMethods)
                ? storeData.paymentMethods.map(
                    (pm: any) =>
                      pm.paymentMethod?.methodName || 'Payment Method'
                  )
                : [],
              contactInfo: {
          ...
          if (module.type === 'business-address') {
            ...
            const getPrimaryAddress = (addresses: any[]): any | null => {
              ...
              const businessAddress = Array.isArray(addresses)
                ? addresses.find((addr) => addr.addressType === 'business')
                : undefined;
    ...
    try {
      ...
      const response = await InventoryApiService.getInventoryItems(
        Number(storeId),
        {
          ...
          pageSize: 50, // Limit for performance
          includeImages: true,
        }
      );
      ...
      const imagePromises = items.slice(0, 6).map(async (item: any) => {
        try {
          const imageResponse =
            await InventoryApiService.getInventoryItemImages(item.itemId);
          const primaryImage = imageResponse?.find((img: any) => img.isPrimary);
          return { itemId: item.itemId, imageUrl: primaryImage?.url };
        } catch {
  ...
  // Load featured products for the storefront
  const loadFeaturedProducts = useCallback(async () => {
    ...
    try {
      ...
      // Create storefront data object for preview
      const mockStorefront = {
        storeId: Number(storeId),
        storeName: storeData?.storeName || 'Custom Butcher',
  ...
  }, [storeId, storeData, inventoryItems]);
  ...
  // Load featured products when store data is available
  useEffect(() => {
    if (storeData) {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'grey.50' }}>
                ...
                {modules
                  ...
                  .map((module) => (
                    ...
                    >
                        ...
                        {module.type === 'featured-products' && (
                          <Box sx={{ display: 'flex', gap: 1 }}>
                            ...
                            <Button
                              ...
                              startIcon={<Star />}
                              onClick={() =>
                                setFeaturedProductsManagerOpen(true)
                              }
                              sx={{
                                textTransform: 'none',
                                fontSize: '0.75rem',
                      ...
                      {module.type === 'featured-products' && (
                        <Box>
                          {/* Use the actual FeaturedProductsModule component for consistent preview */}
                          {storefrontData ? (
                            <div>
                              <Typography
                                variant='h6'
                                ...
                                {storefrontData.products.length > 0 ? (
                                      ...
                                      <Box key={product.productId}>
                                        <Card
                                          sx={{
                                            ...
                                            borderRadius:
                                              selectedTheme.layout.borderRadius
                                                .md,
                                            boxShadow:
                                              selectedTheme.effects.boxShadow
                                                .sm,
                                            border: `1px solid ${selectedTheme.colors.border}`,
                                            transition:
                                              selectedTheme.effects.transition
                                                .fast,
                                            overflow: 'hidden',
                                            '&:hover': {
                                              boxShadow:
                                  ...
                                  >
                                    ...
                                    <Typography
                                      ...
                                      display='block'
                                    >
                                      Add products to your inventory and they
                                      will appear here
                                    </Typography>
                                  </Box>
                                )}
      ...
      {/* Featured Products Manager Dialog */}
