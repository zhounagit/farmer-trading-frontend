// Central export file for all feature routes
export { authRoutes } from './auth/routes';
export { dashboardRoutes } from './dashboard/routes';
export { storesRoutes } from './stores/routes';
export { storefrontRoutes } from './storefront/routes';
export { inventoryRoutes } from './inventory/routes';
export { searchRoutes } from './search/routes';
export { coreRoutes } from './core/routes';
export { referralRoutes } from './referral/routes';

// Export default routes as named exports for convenience
export { default as authRoutesDefault } from './auth/routes';
export { default as dashboardRoutesDefault } from './dashboard/routes';
export { default as storesRoutesDefault } from './stores/routes';
export { default as storefrontRoutesDefault } from './storefront/routes';
export { default as inventoryRoutesDefault } from './inventory/routes';
export { default as searchRoutesDefault } from './search/routes';
export { default as coreRoutesDefault } from './core/routes';
export { default as referralRoutesDefault } from './referral/routes';
