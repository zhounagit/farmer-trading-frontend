...
export const useStoreErrorHandler = () => {
  ...
  const handleStoreApiError = useCallback(
    (error: unknown, options: StoreErrorHandlerOptions = {}): StoreError => {
      const {
        showToast = true,
  ...
  const handleStoreCreationError = useCallback(
    (error: unknown, step?: string): StoreError => {
      ...
      if (storeError.code === 'STORE_NAME_EXISTS') {
        ...
        storeError.message =
          'Invalid store type selected. Please choose a valid store type.';
  ...
  const handleStoreUpdateError = useCallback(
    (error: unknown, operation: string = 'update'): StoreError => {
      const storeError = handleStoreApiError(error, {
        showToast: true,
      ...
      if (storeError.code === 'STORE_NOT_FOUND') {
        storeError.message =
          'Store not found. It may have been deleted or you may not have permission to access it.';
  ...
  const handleMediaUploadError = useCallback(
    (error: unknown, fileType: string = 'file'): StoreError => {
      const storeError = handleStoreApiError(error, {
        showToast: true,
...
function isNetworkError(error: unknown): boolean {
  ...
  if (typeof error === 'object' && error !== null) {
    const err = error as any;
    return err.code === 'NETWORK_ERROR' || err.message?.includes('network');
  }
