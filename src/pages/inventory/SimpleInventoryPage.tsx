...
import type { ProductCategory } from '../../services/category.api';
import {
  Add,
  Edit,
  Delete,
...
import type { InventoryImage as ApiInventoryImage } from '../../services/inventory.api';
...
const SimpleInventoryPage: React.FC = () => {
  ...
  const loadItems = useCallback(async () => {
    try {
      ...
      console.log('🔍 DEBUG - loadItems - Response data:', response.data);
      console.log(
        '🔍 DEBUG - loadItems - Response data length:',
      ...
      if (response.success && response.data) {
        ...
        console.log('❌ Failed to load items - response not successful');
        setItems([]);
      }
    } catch (error) {
  ...
  const handleUpdateItem = async () => {
    try {
      ...
      await inventoryApi.updateInventoryItem(selectedItem.itemId, selectedItem);
  ...
  const loadItemImages = async (itemId: number) => {
    ...
    try {
      const response = await inventoryApi.getInventoryImages(itemId);
      console.log('🔍 DEBUG - loadItemImages - API response:', response);
      console.log('🔍 DEBUG - loadItemImages - Response data:', response.data);
      if (response.success && response.data) {
        console.log(
          '🔍 DEBUG - loadItemImages - Setting images:',
          response.data
        );
        console.log(
          '🔍 DEBUG - loadItemImages - First image originalUrl:',
          response.data[0]?.originalUrl
        );
        setItemImages(response.data);
      } else {
        console.log(
          '🔍 DEBUG - loadItemImages - No images found or response failed'
        );
        setItemImages([]);
      }
    } catch (error) {
  ...
  const handleImageUpload = async (files: FileList) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          const response = await inventoryApi.uploadInventoryImage({
            itemId: currentItemForImages.itemId,
            image: file,
            isPrimary: itemImages.length === 0,
            altText: file.name,
            displayOrder: itemImages.length + i + 1,
          });
          ...
          if (response.success) {
            ...
            if (response.data) {
              setItemImages((prev) => [...prev, response.data]);
            }
          }
        } catch (error) {
  ...
  const handleDeleteImage = async (imageId: number) => {
    ...
    try {
      await inventoryApi.deleteInventoryImage(
