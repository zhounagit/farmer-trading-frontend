...
const ProductSearchPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (sortFromUrl && sortOptions.some((opt) => opt.value === sortFromUrl)) {
  ...
  useEffect(() => {
    const searchProducts = async () => {
      ...
      try {
        ...
        storefrontResponse.storefronts.forEach((storefront) => {
                    ...
                    ? [
                        {
                          imageUrl: product.imageUrl.startsWith('http')
                            ? product.imageUrl
                            : `https://localhost:7008${product.imageUrl}`,
  ...
  const getPrimaryImage = (product: ProductSearchResult) => {
    ...
    return primaryImage?.imageUrl || '/placeholder-product.jpg';
  };
