...
const UnifiedSearchPage: React.FC = () => {
  ...
  const handleSuggestionClick = (suggestion: SearchSuggestion) => {
    ...
    if (suggestion.entityType === 'store' && suggestion.entityId) {
      navigate(`/storefront/${suggestion.entityId}`);
      return;
    }
  ...
  const renderSearchResult = (result: SearchResult, index: number) => {
    const isProduct = result.entityType === 'product';
    const isStore = result.entityType === 'store';
    const isCategory = result.entityType === 'category';
    ...
    return (
      ...
      >
        <Card
          ...
          onClick={() => {
            if (isStore) {
              navigate(`/store/${result.storeSlug || result.storeId}`);
            } else if (isProduct) {
              navigate(`/product/${result.entityId}`, {
                state: {
              ...
              setFilters((prev) => ({ ...prev, category: result.primaryName }));
            }
          }}
        >
          {result.imageUrl && (
            ...
            <Stack spacing={1}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                {result.relevanceScore > 0 && (
                  <Chip
                    label={`${Math.round(result.relevanceScore)}%`}
                    size='small'
                    color='default'
                    sx={{ ml: 'auto' }}
                  />
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  {result.quantity !== undefined && (
                    <Typography
                      variant='body2'
                      color={
                        result.quantity > 0 ? 'success.main' : 'error.main'
                      }
                      sx={{ ml: 'auto' }}
                    >
                      {result.quantity > 0
                  ...
                  >
                    {suggestions.map((suggestion, index) => (
                      ...
                      >
                        ...
                        <Chip label={suggestion.matchCount} size='small' />
                      </Box>
                    ))}
                  </Paper>
                )}
