...
type SellingMethod = 'on-farm-pickup' | 'local-delivery' | 'farmers-market' | 'processor-pickup';
...
interface PartnershipLogistics {
  ...
  deliveryArrangement: 'producer-delivers' | 'customer-delivers' | 'pickup-service';
...
const LocationLogisticsStepEnhanced: React.FC<StepProps> = ({
  ...
  const [processorPartners, setProcessorPartners] = useState<ProcessorPartner[]>([]);
  const [selectedProcessors, setSelectedProcessors] = useState<number[]>([]);
  const [partnershipLogistics, setPartnershipLogistics] = useState<PartnershipLogistics[]>([]);

  // Determine if this is a producer store that needs processor logistics
  const isProducerStore = formState.storeBasics?.setupFlow?.derivedStoreType === 'producer';
  const hasLiveAnimalsCategory = formState.storeBasics?.categories?.includes('Live Animals');
  ...
  const loadProcessorPartners = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const preSelectedIds = formState.storeBasics?.setupFlow?.selectedPartnerIds || [];
        ...
        const initialLogistics = preSelectedIds.map(id => {
          const partner = partners.find(p => p.storeId === id);
          return {
            processorId: id,
        ...
        console.log('✅ Successfully loaded processor partners via direct fetch');
      } else {
  ...
  const validateSellingMethods = () => {
    ...
    if (formState.locationLogistics.sellingMethods.includes('processor-pickup')) {
      if (selectedProcessors.length === 0) {
        newErrors.processorSelection = 'Please select at least one processor partner';
      }

      // Validate each processor's logistics
      partnershipLogistics.forEach((logistics, index) => {
        if (!logistics.deliveryArrangement) {
          newErrors[`processor_${logistics.processorId}_delivery`] = 'Please specify delivery arrangement';
        }
        if (logistics.processingTimeframes.length === 0) {
          newErrors[`processor_${logistics.processorId}_timeframe`] = 'Please specify processing timeframe';
        }
      });
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await OpenShopApiService.createStoreAddress(formState.storeId, {
        ...
        ContactPhone: formatPhoneNumber(formState.locationLogistics.businessAddress.contactPhone),
        ContactEmail: formState.locationLogistics.businessAddress.contactEmail,
        StreetAddress: formState.locationLogistics.businessAddress.streetAddress,
      ...
      if (sellingMethods.includes('on-farm-pickup')) {
        if (!formState.locationLogistics.farmgateSameAsBusinessAddress) {
          await OpenShopApiService.createStoreAddress(formState.storeId, {
            ...
            LocationName: formState.locationLogistics.farmgateAddress?.locationName || 'Farm Gate',
            ContactPhone: formatPhoneNumber(formState.locationLogistics.farmgateAddress?.contactPhone || ''),
            StreetAddress: formState.locationLogistics.farmgateAddress?.streetAddress || '',
            ...
            Country: formState.locationLogistics.farmgateAddress?.country || 'US',
            IsPrimary: false,
            IsActive: true,
          });
        }
      ...
      if (sellingMethods.includes('local-delivery')) {
        const deliveryRadius = formState.locationLogistics.deliveryRadiusMi || 5;
        await OpenShopApiService.setDeliveryDistance(formState.storeId, deliveryRadius);
      }
      ...
      if (sellingMethods.includes('farmers-market')) {
        await OpenShopApiService.createStoreAddress(formState.storeId, {
          ...
          LocationName: formState.locationLogistics.pickupPointAddress?.locationName || 'Pickup Location',
          ContactPhone: formatPhoneNumber(formState.locationLogistics.pickupPointAddress?.contactPhone || ''),
          StreetAddress: formState.locationLogistics.pickupPointAddress?.streetAddress || '',
          City: formState.locationLogistics.pickupPointAddress?.city || '',
          State: formState.locationLogistics.pickupPointAddress?.state || '',
          ZipCode: formState.locationLogistics.pickupPointAddress?.zipCode || '',
          Country: formState.locationLogistics.pickupPointAddress?.country || 'US',
          IsPrimary: false,
          IsActive: true,
        });
      }
      ...
      if (sellingMethods.includes('local-delivery')) {
        ...
        await OpenShopApiService.createShippingService(formState.storeId, standardShipping);
      }
      ...
      toast.error('Failed to save location and logistics settings. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };

  const handleSellingMethodChange = (method: SellingMethod, checked: boolean) => {
  ...
  const handleProcessorSelectionChange = (processorId: number, selected: boolean) => {
    ...
    if (selected) {
      const processor = processorPartners.find(p => p.storeId === processorId);
      ...
      setPartnershipLogistics(partnershipLogistics.filter(l => l.processorId !== processorId));
    }
  ...
  const updateProcessorLogistics = (processorId: number, field: keyof PartnershipLogistics, value: any) => {
  ...
  const isOnFarmPickupSelected = formState.locationLogistics.sellingMethods.includes('on-farm-pickup');
  const isLocalDeliverySelected = formState.locationLogistics.sellingMethods.includes('local-delivery');
  const isFarmersMarketSelected = formState.locationLogistics.sellingMethods.includes('farmers-market');
  const isProcessorPickupSelected = formState.locationLogistics.sellingMethods.includes('processor-pickup');
  ...
  return (
    <Box sx={{ maxWidth: 800, mx: 'auto', p: 3 }}>
      ...
      <Typography variant="body1" color="text.secondary" paragraph>
        Set up where your business operates and how customers will receive their orders.
        {needsProcessorLogistics && (
          <Alert severity="info" sx={{ mt: 2 }}>
            <Typography variant="body2">
              Since you're selling live animals, you can set up partnerships with processors
              so customers can pick up processed meat directly from processor facilities.
            </Typography>
          </Alert>
        )}
      </Typography>
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={(e) => handleSellingMethodChange('on-farm-pickup', e.target.checked)}
                color="primary"
              />
            }
            label={
              <Box>
                <Typography variant="body1">On-Farm Pickup (Customers come to me)</Typography>
                <Typography variant="body2" color="text.secondary">
                  Customers visit your farm to pick up their orders
                </Typography>
              </Box>
            }
          />
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={(e) => handleSellingMethodChange('local-delivery', e.target.checked)}
                color="primary"
              />
            }
            label={
              <Box>
                <Typography variant="body1">Local Delivery (I deliver to customers)</Typography>
                <Typography variant="body2" color="text.secondary">
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={(e) => handleSellingMethodChange('farmers-market', e.target.checked)}
                color="primary"
              />
            }
            label={
              <Box>
                <Typography variant="body1">Farmers Market / Pickup Point</Typography>
                <Typography variant="body2" color="text.secondary">
                  Customers pick up at farmers markets or designated pickup locations
                </Typography>
              </Box>
            }
          />
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(e) => handleSellingMethodChange('processor-pickup', e.target.checked)}
                  color="primary"
                />
              }
              label={
                <Box>
                  <Typography variant="body1">
                    <ProcessorIcon sx={{ mr: 1, verticalAlign: 'middle' }} />
                    Processor Pickup (Customers pick up processed meat from processors)
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Customers get processed products from your processor partners
                  </Typography>
                </Box>
              }
            />
      ...
      {isProcessorPickupSelected && needsProcessorLogistics && (
        <Paper elevation={1} sx={{ p: 4, mb: 4, borderRadius: 2 }}>
          <Typography variant="h5" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="body2" color="text.secondary" paragraph>
            Configure how customers will interact with your processor partners for pickup and processing.
          </Typography>
          ...
          <List sx={{ mb: 3 }}>
            {processorPartners.map((processor) => (
              <ListItem key={processor.storeId} divider>
                <ListItemAvatar>
                  <Avatar src={processor.logoUrl} sx={{ bgcolor: 'primary.light' }}>
                    <ProcessorIcon />
                  </Avatar>
                </ListItemAvatar>
                <ListItemText
                ...
                <ListItemSecondaryAction>
                  <Switch
                    ...
                    onChange={(e) => handleProcessorSelectionChange(processor.storeId, e.target.checked)}
                    color="primary"
                  />
                </ListItemSecondaryAction>
              </ListItem>
          ...
          {partnershipLogistics.map((logistics) => (
            <Card key={logistics.processorId} sx={{ mb: 3, border: '1px solid', borderColor: 'divider' }}>
              <CardContent>
                ...
                <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' } }}>
                  <FormControl component="fieldset">
                    <FormLabel component="legend">How do animals get to the processor?</FormLabel>
                    <RadioGroup
                      value={logistics.deliveryArrangement}
                      onChange={(e) => updateProcessorLogistics(logistics.processorId, 'deliveryArrangement', e.target.value)}
                    >
                      <FormControlLabel
                  ...
                  <FormControl component="fieldset">
                    <FormLabel component="legend">Processing Timeframes</FormLabel>
                    <FormGroup>
                      {['Same day', '1-2 days', '3-5 days', '1 week', 'Custom schedule'].map((timeframe) => (
                        <FormControlLabel
                          key={timeframe}
                          control={
                            <Checkbox
                              checked={logistics.processingTimeframes.includes(timeframe)}
                              onChange={(e) => {
                                const newTimeframes = e.target.checked
                                  ? [...logistics.processingTimeframes, timeframe]
                                  : logistics.processingTimeframes.filter(t => t !== timeframe);
                                updateProcessorLogistics(logistics.processorId, 'processingTimeframes', newTimeframes);
                              }}
                            />
                          }
                          label={timeframe}
                  ...
                  <FormControl component="fieldset">
                    <FormLabel component="legend">Customer Notification Method</FormLabel>
                    <RadioGroup
                      value={logistics.customerNotificationMethod}
                      onChange={(e) => updateProcessorLogistics(logistics.processorId, 'customerNotificationMethod', e.target.value)}
                    >
                      <FormControlLabel value="email" control={<Radio />} label="Email notification" />
                      <FormControlLabel value="sms" control={<Radio />} label="SMS notification" />
                  ...
                  <TextField
                    ...
                    onChange={(e) => updateProcessorLogistics(logistics.processorId, 'specialInstructions', e.target.value)}
                ...
                {Object.entries(errors).filter(([key]) => key.startsWith(`processor_${logistics.processorId}_`)).map(([key, error]) => (
