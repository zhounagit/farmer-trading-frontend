...
import { AVAILABLE_THEMES, generateThemeCSS, type StorefrontTheme } from '../../../types/themes';
...
const ThemeTester: React.FC<ThemeTesterProps> = ({
  ...
  useEffect(() => {
    ...
    const extractedVars = extractCSSVariables(css);
    setCssVariables(extractedVars);
  ...
  const testCSSVariable = (varName: string): 'success' | 'warning' | 'error' => {
    try {
  ...
  const getVariableValue = (varName: string): string => {
    try {
      return getComputedStyle(document.documentElement).getPropertyValue(varName).trim() || 'Not set';
    } catch {
      return 'Error';
    }
  };
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            ...
            >
              This card tests surface colors, shadows, and border radius from the current theme.
            </Typography>
          </Paper>
        </Grid>
      </Grid>
